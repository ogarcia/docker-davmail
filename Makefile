...
DAVMAIL_VERSION := 6.2.2
DAVMAIL_REVISION := 3546
...
container-buildx-push: check-quay-env check-github-registry-env
	...
	docker buildx build $(TAGS) --platform $(CONTAINER_ARCHITECTURES) --build-arg ALPINE_VERSION=$(ALPINE_VERSION) --build-arg DAVMAIL_VERSION=$(DAVMAIL_VERSION) --build-arg DAVMAIL_REVISION=$(DAVMAIL_REVISION) --push .
